...
(() => {
  ...
  const stylesheet =
    ...
    '  pointer-events:none}' +
    // Re-enable pointer events on the image only while reframing — that way
    // horizontal swipes inside scroll-snap sliders reach the parent track on
    // touch devices instead of being eaten by the image element.
    ':host([data-reframe]) .frame img{pointer-events:auto;touch-action:none}' +
